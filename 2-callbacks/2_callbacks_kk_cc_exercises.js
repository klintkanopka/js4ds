...
// This section sticks Method 1 into a named function annotate()
annotate = (input) => {
  let annotated = input
             .reduce((accum, row) => {
                 const newRow = {
...
// This section sticks Method 1 into a named function annotate2()
annotate2 = (input) => {
  let accum = []
  input.reduce((accum, row) => {
      const newRow = {
          'seq': accum.length + 1,
          'year': row.date.split('-')[0],
          'sex': row.sex,
      ...
      accum.push(newRow)
      return accum }, accum) // tell it to add newRow to accum
    return accum.filter((row) => {return row.sex == 'F'})
}

console.log('new data:')
newData.forEach((row) => {console.log(row)})

// print results of annotate()
result = annotate(data)
console.log(JSON.stringify(result))

// print results of annotate2()
result2 = annotate2(data)
